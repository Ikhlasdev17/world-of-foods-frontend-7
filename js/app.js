...
function fetchCategories() {
	arr.map(item => {
		...
		<div role='status' class="max-w-sm animate-pulse">
			<div class=" bg-gray-50 rounded-md dark:bg-gray-200 w-full mb-4 p-4 flex items-center gap-4">
				<div class='w-[50px] h-[50px] bg-gray-100 dark:bg-gray-300 rounded-md'></div>
				<div class='w-[100px] h-[30px] rounded-full bg-gray-100 dark:bg-gray-300'></div>
			</div>
		</div>
		<!-- skeleton loader end -->
		`
...
function fetchMeals(categoryElement) {
	...
	arr.map(item => {
		...
		<!-- single skeleton card item end -->
		`
	})
	selectedCategory = categoryElement?.dataset?.category
	fetch(
		`https://www.themealdb.com/api/json/v1/1/filter.php?c=${
			categoryElement?.dataset?.category || 'Beef'
		}`
	)
		.then(res => res.json())
